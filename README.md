...
The fastest way to get up and running is using our single-line bootstrap command. This safely clones the repository into a temporary directory and launches the interactive installer — pick a **PC** (full desktop) or **Server** (headless) profile and it handles the rest.
...
- **Two Setup Profiles**: Choose **PC** (full desktop: Hyprland, SDDM, Waybar + all dev tools) or **Server** (headless: terminal & dev tools only) and the installer picks exactly the right packages and tool scripts.
- **No Partial Upgrades**: Performs a full `pacman -Syu` system refresh before installing anything, keeping your machine consistent and preventing stale-database 404 errors.
...
- **Core CLI Tools**: `neovim`, `tmux`, `git`, `zoxide`, `eza`, `fzf`, `fd`, `ripgrep`, `bat`, `btop`, `yazi`
...
- **Formatting & Parsing**: `tree-sitter-cli`, `stylua`, `taplo-cli`, `yamlfmt`, `shfmt`, `shellcheck`
...
│   ├── tree-sitter-cli.sh # Verifies tree-sitter CLI
│   ├── uv.sh           # Installs uv + Python tools (ty, ruff)
...
- **Packages**: Add new packages to `ALL_PACKAGES` in `setup.sh`. If they make sense on a headless machine, add them to `SERVER_PACKAGES` too.
...
- **Tools**: Add new `.sh` scripts to the `tools/` directory and append their names to `TOOL_ORDER` (and `SERVER_TOOLS` if server-safe) in `setup.sh`.
